...
if [[ -z $* ]]; then
	echo 'No argument found. All folders known to have tests will be tested. (mgi, api, user_dashboard, explore_tree, schema_viewer, utils/XSDhash)'
	python manage.py test mgi/ user_dashboard/ api/ explore_tree/ schema_viewer/ utils/XSDhash --liveserver=localhost:8082 --no-selenium
else
