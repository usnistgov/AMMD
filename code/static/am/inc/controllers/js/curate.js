...
changeHTMLForm = function(operation, tagID)
{
    ...
    if (operation == 'add') {
        ...
        $("#element"+tagID).children("div.module").length == 0){ // module not generated
            ...
            $("#element"+tagID).children(".expand").attr("class", "collapse-btn");
            duplicate(tagID);
        }
    } else if (operation == 'remove') {
        $("#element"+tagID).children(".collapse-btn").attr("class","expand");
        remove(tagID);
    }
...
disable_element = function(tagID){
    $("#" + tagID).children(".collapse-btn").attr("class","expand");
...
setCurrentTemplate = function()
{
    ...
    tdElement.html('<span><i class="fa fa-spinner fa-pulse fa-fw"></i> Loading...</span>');
